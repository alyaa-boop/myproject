...
<div class="container mx-auto px-4 md:px-6 py-8">
    ...
    <div class="space-y-8">
        <div class="text-center">
            <h1 class="text-3xl font-bold text-gray-900">{{ $content['title'] ?? 'Carta Organisasi Alumni 4B Malaysia' }}</h1>
            <p class="text-gray-600 mt-2 max-w-2xl mx-auto">
                {{ $content['subtitle'] ?? 'Struktur organisasi Alumni 4B Malaysia yang menguruskan aktiviti dan program di peringkat kebangsaan dan negeri.' }}
            </p>
        </div>
        ...
        <div class="bg-gray-100 p-6 rounded-lg">
            <div class="flex flex-col items-center">
                <div class="bg-primary text-white px-6 py-3 rounded-lg text-center mb-4 w-64">
                    ...
                    <p class="text-sm">{{ $content['pengerusi'] ?? "Dato' Ahmad bin Abdullah" }}</p>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div class="bg-primary/90 text-white px-4 py-2 rounded-lg text-center">
                        ...
                        <p class="text-xs">{{ $content['tp'] ?? 'Puan Siti binti Hassan' }}</p>
                    </div>
                    <div class="bg-primary/90 text-white px-4 py-2 rounded-lg text-center">
                        <h3 class="font-bold text-sm">Setiausaha</h3>
                        <p class="text-xs">{{ $content['setiausaha'] ?? 'Encik Ravi a/l Chandran' }}</p>
                    </div>
                    <div class="bg-primary/90 text-white px-4 py-2 rounded-lg text-center">
                        <h3 class="font-bold text-sm">Bendahari</h3>
                        <p class="text-xs">{{ $content['bendahari'] ?? 'Puan Lim Mei Ling' }}</p>
                    </div>
                </div>
                <div class="w-0.5 h-6 bg-gray-300"></div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 w-full max-w-3xl">
                    <div class="bg-primary/80 text-white px-4 py-2 rounded-lg text-center">
                        ...
                        <p class="text-xs">{{ $content['jk_keahlian'] ?? 'Encik Azman bin Aziz' }}</p>
                    </div>
                    <div class="bg-primary/80 text-white px-4 py-2 rounded-lg text-center">
                        <h3 class="font-bold text-sm">Jawatankuasa Aktiviti</h3>
                        <p class="text-xs">{{ $content['jk_aktiviti'] ?? 'Puan Nurul Huda binti Ismail' }}</p>
                    </div>
                    <div class="bg-primary/80 text-white px-4 py-2 rounded-lg text-center">
                        <h3 class="font-bold text-sm">Jawatankuasa Perhubungan</h3>
                        <p class="text-xs">{{ $content['jk_perhubungan'] ?? 'Encik Chong Kok Wai' }}</p>
                    </div>
                </div>
                <div class="w-0.5 h-6 bg-gray-300"></div>
        ...
        @php $executiveCommittee = !empty($content['executive']) ? $content['executive'] : \App\Models\PageContent::defaultContent('carta-organisasi')['executive'] ?? []; @endphp
        <div>
        ...
        {{-- Jawatankuasa tabs --}}
        @php $committees = !empty($content['committees']) ? $content['committees'] : (\App\Models\PageContent::defaultContent('carta-organisasi')['committees'] ?? ['Keahlian' => [], 'Aktiviti' => [], 'Perhubungan' => []]); @endphp
        ...
        {{-- Jawatankuasa Negeri --}}
        @php $stateChapters = !empty($content['state_chapters']) ? $content['state_chapters'] : (\App\Models\PageContent::defaultContent('carta-organisasi')['state_chapters'] ?? []); @endphp
        <div>
