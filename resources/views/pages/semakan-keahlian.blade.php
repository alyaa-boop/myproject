...
<div class="container mx-auto px-4 md:px-6 py-8">
    ...
    <div class="max-w-3xl mx-auto" x-data="semakanKeahlian">
        ...
        <div class="rounded-lg border bg-white shadow-sm overflow-hidden">
            ...
            <div class="p-6">
                <form method="POST" action="{{ route('semakan-keahlian.post') }}" @submit.prevent="search()" class="space-y-6">
                    @csrf
                    <div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex gap-2">
                            <input type="text" x-model="searchValue" name="value" :placeholder="searchMethod === 'ic' ? '123456789012' : 'A00001'" class="flex-1 rounded-md border border-gray-300 px-3 py-2 text-sm focus:border-primary focus:ring-1 focus:ring-primary">
                            <input type="hidden" name="method" :value="searchMethod">
                    ...
                    {{-- Pending --}}
                    <div x-show="result && result.status === 'pending'" x-cloak class="rounded-lg border border-yellow-200 bg-yellow-50 p-4 flex gap-3 mt-6">
                        <svg class="h-5 w-5 text-yellow-600 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
                        <div>
                            <p class="font-medium text-yellow-800">Permohonan Dalam Proses</p>
                            <p class="text-sm text-yellow-700" x-text="result.message || 'Permohonan anda masih dalam proses pengesahan.'"></p>
                        </div>
                    </div>
...
document.addEventListener('alpine:init', () => {
  Alpine.data('semakanKeahlian', () => ({
    ...
    async search() {
      ...
      formData.append('method', this.searchMethod);
      formData.append('value', this.searchValue.trim().replace(/\s/g, ''));
      try {
        const res = await fetch('{{ route("semakan-keahlian.post") }}', {
          method: 'POST',
          body: formData,
